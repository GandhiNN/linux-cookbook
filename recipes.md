...
## 6. Extracting TLS certificate(s) that a networking node presents during the handshake

```bash
openssl s_client -connect <host:port> -showcerts < /dev/null 2>/dev/null | sed -n '/BEGIN CERTIFICATE/,/END CERTIFICATE/p' > certs-real/cacerts.pem 
```

Breakdown:

1. `openssl s_client -connect host:port -showcerts` - Initiates a TLS handshake with the host and prints all certificates in the chain (server cert + intermediate CAs + possibly root CA).
2. `< / dev/null` - Closes stdin immediately so it does not hang waiting for input.
3. `sed -n '/BEGIN CERTIFICATE/,/END CERTIFICATE/p'` - Extracts only the PEM-encoded certificate blocks, stripping the diagnostic text.
4. `> certs-real/cacerts.pem` - Saves them as the CA file that an application uses for TLS verification.
